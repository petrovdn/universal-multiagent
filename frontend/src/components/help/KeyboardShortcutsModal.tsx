...
export function KeyboardShortcutsModal({ isOpen, onClose }: KeyboardShortcutsModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const cacheKey = 'KEYBOARD_SHORTCUTS.md'
      ...
      fetch('/docs/KEYBOARD_SHORTCUTS.md', { signal: abortControllerRef.current.signal })
        .then((res) => {
        ...
        .catch((err) => {
          ...
          setContent('# Ошибка загрузки\n\nНе удалось загрузить горячие клавиши.')
          setIsLoading(false)
        })
    } else {
    ...
    <div className="documentation-modal-overlay" onClick={onClose}>
      <div className="documentation-modal" onClick={(e) => e.stopPropagation()}>
        <div className="documentation-modal-header">
          <h2 className="documentation-modal-title">Горячие клавиши</h2>
          <button
